...
`.trim();
    return (await callGeminiForText(prompt, "review:self", 1200, REVIEW_MODEL_CHAIN)).trim();
  }
...
`.trim();
  return (await callGeminiForText(prompt, "review:team", 1800, REVIEW_MODEL_CHAIN)).trim();
}
