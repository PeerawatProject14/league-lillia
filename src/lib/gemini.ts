...
async function callGeminiForText(prompt: string, label: string, maxTokens = 600, modelChain: string[] = BUILD_MODEL_CHAIN): Promise<string> {
  const client = getGeminiClient();
  let lastErr: any = null;
...
`.trim();
    return (await callGeminiForText(prompt, "review:self", 6000, REVIEW_MODEL_CHAIN)).trim();
  }
...
`.trim();
  return (await callGeminiForText(prompt, "review:team", 8000, REVIEW_MODEL_CHAIN)).trim();
}
